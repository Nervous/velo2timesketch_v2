...
process_files () {
    ...
    # Create plaso folder in case it doesn't exist
    docker exec -i timesketch-worker /bin/bash -c "mkdir /usr/share/timesketch/upload/plaso_complete"
    docker exec -i timesketch-worker /bin/bash -c "mkdir /usr/share/timesketch/upload/plaso"
    ...
    # Install timesketch-import-client as it is not imported by default?
    docker exec -i timesketch-worker /bin/bash -c "pip install timesketch-import-client"
    ...
    if [[ "$LABEL" == "ir-"* ]]; then
        if [[ "$SKETCH_ID" == "none" ]]; then    
                docker exec -i timesketch-worker /bin/bash -c "timesketch_importer -u $USERNAME -p '$PASSWORD' --host http://timesketch-web:5000 --timeline_name $SYSTEM --sketch_name $LABEL /usr/share/timesketch/upload/plaso/$SYSTEM.plaso"
        else
                docker exec -i timesketch-worker /bin/bash -c "timesketch_importer -u $USERNAME -p '$PASSWORD' --host http://timesketch-web:5000 --timeline_name $SYSTEM --sketch_id $SKETCH_ID /usr/share/timesketch/upload/plaso/$SYSTEM.plaso"
        fi
    else        
        docker exec -i timesketch-worker /bin/bash -c "timesketch_importer -u $USERNAME -p '$PASSWORD' --host http://timesketch-web:5000 --timeline_name $SYSTEM --sketch_name $SYSTEM /usr/share/timesketch/upload/plaso/$SYSTEM.plaso"
    fi
